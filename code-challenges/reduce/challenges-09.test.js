...
const countNumberOfElements = (arr) => arr.reduce((a, c) => a + 1, 0);
...
const returnNames = (arr) => arr.reduce((a, c) => a.concat(c.name), []);
...
const reversedString = (arr) => arr.split('').reduce((a, c) => a = c + a, '');
...
const countNumberOfChildren = (arr) => arr.reduce((a, c) => a + (c.children ? c.children.length : 0), 0);
...
const calculateAverage = (arr) => arr.reduce((a, c) => a + c) / arr.length;
...
const countPrimeNumbers = (arr) => arr.reduce((a, c) => a + (isPrime(c) ? 1 : 0) , 0);
