...
const count = (target, input) => input.reduce((a, c) => a + c.reduce((a, c) => a + (c === target), 0), 0);
...
const totalSum = (input) => input.reduce((a, c) => a + c.reduce((a, c) => a + c, 0), 0);
...
const divisibleByFiveTwoToThePower = (input) => input.map(e => e.filter(e => (typeof e === 'number' && e % 5 === 0)).map(e => 2**e));
...
let findMaleAndFemale = (data) => data.filter(e => /male/.test(e.gender)).map(e => e.name).reduce((a, c) => `${a} and ${c}`);
...
let findShortest = (data) => data.reduce((a, c) => (parseInt(a.height) < parseInt(c.height)) ? a : c).name;
