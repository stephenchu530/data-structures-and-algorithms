...
const forEachTwoToThe = (arr) => {
  // Solution code here...
  let result = [];
  arr.forEach(pow => result.push(2**pow));
  return result;
};
...
const mapTwoToThe = (arr) => arr.map(pow => 2**pow);
...
const charCode = (arr) => arr.map(chr => chr.charCodeAt());
...
const extractStats = (arr) => arr.map(skill => {
  return {
    name: skill.stat.name,
    total: skill.effort + skill.baseStat
  };
});
