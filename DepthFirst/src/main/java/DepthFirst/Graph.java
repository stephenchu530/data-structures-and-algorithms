...
public class Graph<T> {
    ...
    public ArrayList<T> depthFirst(GraphNode<T> startNode) {
        ArrayList<T> results = new ArrayList<>();
        HashSet<T> visited = new HashSet<>();

        visited.add(startNode.getValue());
        depthFirstHelper(startNode, results, visited);

        return results;
    }

    private void depthFirstHelper(GraphNode<T> node, ArrayList<T> results, HashSet<T> visited) {
        results.add(node.getValue());
        for (GraphNode<T> neighborNode: node.getNeighbors()) {
            if (visited.add(neighborNode.getValue()))
                depthFirstHelper(neighborNode, results, visited);
        }
    }

    public String[] getEdge(Graph<T> graph, T[] arr) {
    ...
    public GraphNode<T> findNode(Graph<T> graph, T value) {
