...
const firstLetters = (arr) => arr.map(e => e.charAt(0));
...
const findHappiness = (arr) => arr.filter(e => e.includes(':)'));
...
const standardizePhoneNumbers = (arr) => arr.map(e => e.match(/\d/g).join(''));
...
const onlyOddChars = (str) => str.split('').filter((e, i) => i % 2).join('');
...
const allHappy = (arr) => arr.every(s => s.includes(':)'));
...
const findEvery = (arr, target) => arr.every(s => s.includes(target));
...
const unenrollBrook = (arr) => arr.map(e => e.filter(s => !s.includes('Brook')));
...
const characterByIndex = (arr) => arr.map((s, i) => s.charAt(i));
