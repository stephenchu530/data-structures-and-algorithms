...
const filterStringsWithVowels = (arr) => arr.filter(word => word.match(/[aeiou]/));;
...
const notInFirstArray = (forbiddenValues, arr) => arr.filter(ele => !forbiddenValues.includes(ele));
...
const getBaseStatGreaterThan = (arr, minBaseStat) => arr.filter(ele => ele.baseStat > minBaseStat);
...
const getStatName = (arr, minBaseStat) => arr.filter(ele => ele.baseStat > minBaseStat).map(ele => ele.stat.name);
...
const getCharactersWithoutChildren = (arr) => arr.filter(ele => !ele.hasOwnProperty('children'));
...
const evenOddNumericValues = (arr) => arr.filter(ele => typeof(ele) === 'number').map(ele => (ele % 2) ? 'odd' : 'even');
