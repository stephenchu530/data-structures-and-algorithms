...
const toTitleCase = (arr) => arr.map(e => e.charAt(0).toUpperCase() + e.slice(1));
...
let biggerThanLuke = (arr) => arr.filter(e => Number(e.mass) > Number(arr[0].mass)).map(e => e.name).join(' - ');
...
const sortBy = (property, arr) => (property === 'name') ? arr.sort((a, b) => a.name < b.name ? -1 : 1) : arr.sort((a, b) => a.price - b.price);
...
const detectTicTacToeWin = (board) => {
  const test = a => /(X{3}|O{3})/.test(a.join(''));
  let rowTest = board.map(e => test(e)).reduce((a, c) => a || c);
  let diagTest = test([board[0][0], board[1][1], board[2][2]]) || test([board[0][2], board[1][1], board[2][0]]);
  let colTest = false;
  for (let i = 0; i < board.length; i++) {
    let col = test([board[0][i], board[1][i], board[2][i]]);
    colTest = colTest || col;
  }
  return rowTest || colTest || diagTest;
};
